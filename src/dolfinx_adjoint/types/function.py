...
def assign(
    value: typing.Union[numpy.inexact, float, int],
    target: typing.Union[Function, Constant],
    **kwargs: typing.Unpack[ad_kwargs],
    ...
    Args:
        ...
        target: The function to assign the value to.
    ...
    annotate = annotate_tape(kwargs) and value != target
    if annotate:
        if not isinstance(value, ufl.core.operator.Operator):
            value = create_overloaded_object(value)
        block = FunctionAssignBlock(value, target, ad_block_tag=ad_block_tag)
    ...
    with stop_annotating():
        if isinstance(target, Function):
            if isinstance(value, (numpy.inexact, float, int)):
                target.x.array[:] = value
            elif isinstance(value, dolfinx.fem.Function):
                assert value.function_space == target.function_space
                target.x.array[:] = value.x.array[:]
            else:
                raise ValueError(f"Unsupported value type for assignment to Function: {type(value)})")
        elif isinstance(target, Constant):
            # Update the underlying value of the Constant
            if hasattr(value, "value"):  # if assigning another constant
