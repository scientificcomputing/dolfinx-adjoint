...
def _hessian_and_fd(n=8, with_coefficient=False):
    ...
    problem = dxa.LinearProblem(
        ...
        bcs=[homogeneous_bc(V)],
        petsc_options=LU,
        petsc_options_prefix="rev_h_",
    )
    uh = problem.solve()
...
def test_B_shape_hessian_is_rank_dependent():
    ...
    if COMM.size == 1:
        ...
        assert abs(Hh - fd) > 1e-3, f"expected a parallel discrepancy, got hessian {Hh} vs fd {fd} on {COMM.size} ranks"
...
def test_D_same_quantity_in_the_rhs_is_refused():
    ...
    with pytest.raises(NotImplementedError, match="differentiates every geometric quantity"):
        dxa.LinearProblem(
            ...
            bcs=[homogeneous_bc(V)],
            petsc_options=LU,
            petsc_options_prefix="rev_d_rhs_",
        ).solve()
...
def test_F_tracking_the_mesh_first_is_required_and_now_checked():
    """Finding F, now addressed: the drift is refused instead of being left to the user.

    Tracking before the first form is still load-bearing -- a form built earlier does not
    depend on the mesh, so a replay re-runs it on whatever geometry the previous replay left
    behind. What has changed is that this no longer drifts silently: ``move`` refuses to move a
    mesh that has such a form on the tape.
    """
    ...
    with pytest.raises(RuntimeError, match="built before the mesh was annotated"):
        _replay_vs_rebuild(annotate_first=False)
...
def test_H_problem_mesh_cache_can_disagree_with_the_block():
    ...
    problem = dxa.LinearProblem(
        ...
        bcs=[homogeneous_bc(V)],
        petsc_options=LU,
        petsc_options_prefix="rev_h_cache_",
    )
    problem.solve()
    ...
    assert any(isinstance(bv.output, Mesh) for bv in block.get_dependencies()), "the block registers the mesh"
    assert not any(isinstance(k, Mesh) for k in templates), "the Problem's TLM templates do not contain it"
...
def test_I_moveblock_aliases_one_adjoint_buffer():
    ...
    second = [b for b in pyadjoint.get_working_tape().get_blocks() if type(b).__name__ == "MoveBlock"][1]
...
def _tear(mesh):
    ...
    local = np.abs(probe.array[n_owned * 3 :] - x[n_owned:, :].reshape(-1)).max() if x.shape[0] > n_owned else 0.0
...
def test_K_move_accepts_a_displacement_from_a_different_mesh():
    ...
    mesh_b = dolfinx.mesh.create_rectangle(COMM, [np.array([0.0, 0.0]), np.array([2.0, 3.0])], [5, 5])
    S_b = dxa.geometry_function_space(mesh_b)
    s_b = dxa.Function(S_b)
    s_b.x.array[:] = 0.1
    before = mesh_a.geometry.x.copy()
    mesh_a = dxa.Mesh(mesh_a)
...
def test_K_a_plain_P1_lookalike_is_accepted_but_happens_to_be_correct():
    ...
    assert W.element == S.element and W.dofmap.index_map_bs == S.dofmap.index_map_bs, "move() cannot tell them apart"
